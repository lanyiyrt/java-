...
底层使用Node类型的数组结构来保存键值对，使用put加入键值对是会调用putval函数来实现，首先判断表是否存在，如果不存在就创建一个新表，默认大小为16，然后计算键值的hash值，对数组大小求余的到索引值，判断该索引位置的是否存在，存在判断该位置的元素是否与要加入的键值的键值和hash值都相等，相等直接更新值，不相等判断是树节点还是链表节点，如果是链表节点需要循环判断是否有个节点与带加入的键相等，没有就直接加入到尾部。同时要判断链表长度是否超过了阈值8，超过了就需要将链表编程红黑树结构。最后判断添加后的表大小是否超过了阈值，超过了还需resize操作。变成两倍，然后两倍，重新计算索引值，从旧表中加入到新表。

**3、如果hashMap的key是一个自定义的类，怎么办？**

需要重写自定义类的hashcode和equals函数，因为不重写这两个函数，默认的是继承Object中的两个函数，这里面的hashcode比较的是两个对象的地址，equals比较的是两个对象是否相等，所以不符合我们的使用的目的。

**4、ArrayList和LinkedList的区别，如果一直在list的尾部添加元素，用哪个效率高？**

两者的底层结构不同，Arraylist使用的是对象数组保存元素，linkedList使用的是双向链表来保存元素，

arraylist在查找的时候支持快速随机查询，但是在插入删除的时候的效率比较低，linkedlist则插入的复杂度O(1),但是不支持随机查询。
